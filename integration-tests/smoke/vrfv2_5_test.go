...
import (
	...
	"github.com/smartcontractkit/chainlink/integration-tests/actions/vrfv2_5"
	"github.com/smartcontractkit/chainlink/integration-tests/actions/vrfv2_5/vrfv2_5_constants"
	"github.com/smartcontractkit/chainlink/integration-tests/docker/test_env"
)

func TestVRFv2_5Billing(t *testing.T) {
	t.Parallel()
	...
	env, err := test_env.NewCLTestEnvBuilder().
		WithGeth().
		WithCLNodes(1).
		WithFunding(vrfv2_5_constants.ChainlinkNodeFundingAmountEth).
		Build()
	...
	mockETHLinkFeedAddress, err := actions.DeployMockETHLinkFeed(env.ContractDeployer, vrfv2_5_constants.LinkEthFeedResponse)
	require.NoError(t, err, "error deploying mock ETH/LINK feed")

	linkAddress, err := actions.DeployLINKToken(env.ContractDeployer)
	require.NoError(t, err, "error deploying LINK contract")

	env, vrfv2_5Contracts, subID, job, err := vrfv2_5.SetupVRFV2_5Environment(env, linkAddress, mockETHLinkFeedAddress)
	require.NoError(t, err, "error setting up VRF v2_5 env")

	subscription, err := vrfv2_5Contracts.Coordinator.GetSubscription(context.Background(), subID)
	...
	t.Run("VRFV2_5 With Link Billing", func(t *testing.T) {
		...
		// test and assert
		err = vrfv2_5Contracts.LoadTestConsumer.RequestRandomness(
			job.KeyHash,
			subID,
			vrfv2_5_constants.MinimumConfirmations,
			vrfv2_5_constants.CallbackGasLimit,
			isNativeBilling,
			vrfv2_5_constants.NumberOfWords,
			vrfv2_5_constants.RandomnessRequestCountPerRequest,
		)
		require.NoError(t, err, "error requesting randomness")

		randomWordsFulfilledEvent, err := vrfv2_5Contracts.Coordinator.WaitForRandomWordsFulfilledEvent([]*big.Int{subID}, nil, time.Minute*2)
		...
		status, err := vrfv2_5Contracts.LoadTestConsumer.GetRequestStatus(context.Background(), randomWordsFulfilledEvent.RequestId)
		require.NoError(t, err, "error getting rand request status")
		require.True(t, status.Fulfilled)
		l.Debug().Interface("Fulfilment Status", status.Fulfilled).Msg("Random Words Request Fulfilment Status")

		require.Equal(t, vrfv2_5_constants.NumberOfWords, uint32(len(status.RandomWords)))
	...
	t.Run("VRFV2_5 With Native Billing", func(t *testing.T) {
