...
func GenerateProofResponseV2_5(keystore keystore.VRF, id string, s PreSeedDataV2_5) (
	vrf_coordinator_v2_5.VRFProof, vrf_coordinator_v2_5.VRFCoordinatorV25RequestCommitment, error) {
	seedHashMsg := append(s.PreSeed[:], s.BlockHash.Bytes()...)
	seed := utils.MustHash(string(seedHashMsg)).Big()
	proof, err := keystore.GenerateProof(id, seed)
	if err != nil {
		return vrf_coordinator_v2_5.VRFProof{}, vrf_coordinator_v2_5.VRFCoordinatorV25RequestCommitment{}, err
	}
	return GenerateProofResponseFromProofV2_5(proof, s)
}
