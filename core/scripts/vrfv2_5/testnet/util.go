...
func deployCoordinator(
	...
	linkTx, err := coordinator.SetLINKAndLINKNativeFeed(e.Owner,
		common.HexToAddress(linkAddress), common.HexToAddress(linkNativeAddress))
	helpers.PanicErr(err)
...
func deployBatchCoordinatorV2(e helpers.Environment, coordinatorAddress common.Address) (batchCoordinatorAddress common.Address) {
	_, tx, _, err := batch_vrf_coordinator_v2_5.DeployBatchVRFCoordinatorV25(e.Owner, e.Ec, coordinatorAddress)
	helpers.PanicErr(err)
	return helpers.ConfirmContractDeployed(context.Background(), e.Ec, tx, e.ChainID)
}
...
func eoaDeployConsumer(e helpers.Environment,
	...
	_, tx, _, err := vrf_v2_5_sub_owner.DeployVRFV25ExternalSubOwnerExample(
		e.Owner,
		e.Ec,
...
func wrapperDeploy(
	...
	address, tx, _, err := vrfv2_5_wrapper.DeployVRFV25Wrapper(e.Owner, e.Ec,
		link,
		linkEthFeed,
		coordinator)
	helpers.PanicErr(err)
	...
	wrapper, err := vrfv2_5_wrapper.NewVRFV25Wrapper(address, e.Ec)
	helpers.PanicErr(err)
...
func wrapperConfigure(
	...
	wrapper, err := vrfv2_5_wrapper.NewVRFV25Wrapper(wrapperAddress, e.Ec)
	helpers.PanicErr(err)
...
func wrapperConsumerDeploy(
	...
	address, tx, _, err := vrfv2_5_wrapper_consumer_example.DeployVRFV25WrapperConsumerExample(e.Owner, e.Ec,
		link,
		wrapper)
	helpers.PanicErr(err)
