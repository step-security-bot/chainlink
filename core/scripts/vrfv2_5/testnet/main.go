...
import (
	...
	"github.com/smartcontractkit/chainlink/v2/core/gethwrappers/generated/vrfv2_5_wrapper"
	"github.com/smartcontractkit/chainlink/v2/core/gethwrappers/generated/vrfv2_5_wrapper_consumer_example"
...
func main() {
	...
	case "manual-fulfill":
		...
		// Sample Loki query:
		// {app="app-name"} | json | taskType="vrfv2_5" |~ "39f2d812c04e07cb9c71e93ce6547e48b7dd23ed4cc02616dfef5ef063a58bde"
	...
	case "batch-coordinatorv2-5-deploy":
		cmd := flag.NewFlagSet("batch-coordinatorv2-5-deploy", flag.ExitOnError)
		...
		_, tx, _, err := batch_vrf_coordinator_v2_5.DeployBatchVRFCoordinatorV25(
			e.Owner, e.Ec, common.HexToAddress(*coordinatorAddr))
		helpers.PanicErr(err)
		helpers.ConfirmContractDeployed(context.Background(), e.Ec, tx, e.ChainID)
	case "batch-coordinatorv2-5-fulfill":
		cmd := flag.NewFlagSet("batch-coordinatorv2-5-fulfill", flag.ExitOnError)
		...
		batchCoordinator, err := batch_vrf_coordinator_v2_5.NewBatchVRFCoordinatorV25(common.HexToAddress(*batchCoordinatorAddr), e.Ec)
		helpers.PanicErr(err)
		...
		proofs := []batch_vrf_coordinator_v2_5.VRFTypesProof{}
		reqCommits := []batch_vrf_coordinator_v2_5.VRFTypesRequestCommitmentV25{}
		for i := range preSeedSlice {
			ps, err := proof.BigToSeed(preSeedSlice[i])
			helpers.PanicErr(err)
			extraArgs, err := extraargs.ExtraArgsV1(*nativePayment)
			helpers.PanicErr(err)
			preSeedData := proof.PreSeedDataV2_5{
				PreSeed:          ps,
				BlockHash:        bhSlice[i],
			...
			finalSeed := proof.FinalSeedV2_5(preSeedData)

			p, err := keyStore.VRF().GenerateProof(*pubKeyHex, finalSeed)
			helpers.PanicErr(err)

			onChainProof, rc, err := proof.GenerateProofResponseFromProofV2_5(p, preSeedData)
			helpers.PanicErr(err)

			proofs = append(proofs, batch_vrf_coordinator_v2_5.VRFTypesProof(onChainProof))
			reqCommits = append(reqCommits, batch_vrf_coordinator_v2_5.VRFTypesRequestCommitmentV25(rc))
		}
		...
		if *estimateGas {
			...
			payload, err := batchCoordinatorV2_5ABI.Pack("fulfillRandomWords", proofs, reqCommits)
			helpers.PanicErr(err)
	...
	case "coordinatorv2-fulfill":
		...
		coordinator, err := vrf_coordinator_v2_5.NewVRFCoordinatorV25(common.HexToAddress(*coordinatorAddr), e.Ec)
		helpers.PanicErr(err)
	...
	case "coordinator-deploy":
		...
		coordinatorDeployLinkNativeFeedAddress := coordinatorDeployCmd.String("link-native-feed", "", "address of link-native-feed")
		helpers.ParseArgs(coordinatorDeployCmd, os.Args[2:], "link-address", "bhs-address", "link-eth-feed")
		deployCoordinator(e, *coordinatorDeployLinkAddress, *coordinatorDeployBHSAddress, *coordinatorDeployLinkNativeFeedAddress)
	...
	case "coordinator-set-config":
		...
		flatFeeNativePPM := cmd.Int64("flat-fee-native-ppm", 500, "fulfillment flat fee native ppm")
	...
	case "consumer-deploy":
		...
		_, tx, _, err := vrf_v2_5_single_consumer.DeployVRFV25SingleConsumerExample(
			e.Owner,
			e.Ec,
	...
	case "consumer-subscribe":
		...
		consumer, err := vrf_v2_5_single_consumer.NewVRFV25SingleConsumerExample(common.HexToAddress(*consumerSubscribeAddress), e.Ec)
		helpers.PanicErr(err)
	...
	case "consumer-cancel":
		...
		consumer, err := vrf_v2_5_single_consumer.NewVRFV25SingleConsumerExample(common.HexToAddress(*consumerCancelAddress), e.Ec)
		helpers.PanicErr(err)
	...
	case "consumer-topup":
		...
		consumer, err := vrf_v2_5_single_consumer.NewVRFV25SingleConsumerExample(common.HexToAddress(*consumerTopupAddress), e.Ec)
		helpers.PanicErr(err)
	...
	case "consumer-request":
		...
		consumer, err := vrf_v2_5_single_consumer.NewVRFV25SingleConsumerExample(common.HexToAddress(*consumerRequestAddress), e.Ec)
		helpers.PanicErr(err)
	...
	case "consumer-print":
		...
		consumer, err := vrf_v2_5_single_consumer.NewVRFV25SingleConsumerExample(common.HexToAddress(*address), e.Ec)
		helpers.PanicErr(err)
	...
	case "deploy-universe":
		deployUniverse(e)
	case "generate-proof-v2-5":
		generateProofForV2_5(e)
	...
	case "eoa-load-test-consumer-with-metrics-deploy":
		...
		_, tx, _, err := vrf_v2_5_load_test_with_metrics.DeployVRFV25LoadTestWithMetrics(
			e.Owner,
			e.Ec,
			common.HexToAddress(*consumerCoordinator),
		)
	...
	case "eoa-request":
		...
		keyHashBytes := common.HexToHash(*keyHash)
		consumer, err := vrf_v2_5_sub_owner.NewVRFV25ExternalSubOwnerExample(
	...
	case "eoa-load-test-request-with-metrics":
		...
		helpers.ParseArgs(request, os.Args[2:], "consumer-address", "sub-id", "key-hash")
		keyHashBytes := common.HexToHash(*keyHash)
		consumer, err := vrf_v2_5_load_test_with_metrics.NewVRFV25LoadTestWithMetrics(
			common.HexToAddress(*consumerAddress),
			e.Ec)
		helpers.PanicErr(err)
		var txes []*types.Transaction
	...
	case "eoa-read":
		...
		consumer, err := vrf_v2_5_single_consumer.NewVRFV25SingleConsumerExample(common.HexToAddress(*consumerAddress), e.Ec)
		helpers.PanicErr(err)
	...
	case "wrapper-deploy":
		...
		helpers.ParseArgs(cmd, os.Args[2:], "link-address", "link-native-feed", "coordinator-address")
		wrapperDeploy(e,
	...
	case "wrapper-withdraw":
		...
		wrapper, err := vrfv2_5_wrapper.NewVRFV25Wrapper(common.HexToAddress(*wrapperAddress), e.Ec)
		helpers.PanicErr(err)
	...
	case "wrapper-consumer-request":
		...
		consumer, err := vrfv2_5_wrapper_consumer_example.NewVRFV25WrapperConsumerExample(
	...
	case "wrapper-consumer-request-status":
		...
		statusStringer := func(status vrfv2_5_wrapper_consumer_example.GetRequestStatus) string {
