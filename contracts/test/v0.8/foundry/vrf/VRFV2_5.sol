 ...
 * Then, pass your request info into the generate-proof-v2-5 script command
 * located in /core/scripts/vrfv2/testnet/proofs.go to generate a proof that can be tested on-chain.
 **/

contract VRFV2_5 is BaseTest {
  address internal constant LINK_WHALE = 0xD883a6A1C22fC4AbFE938a5aDF9B2Cc31b1BF18B;

  // Bytecode for a VRFV2_5_ConsumerExample contract.
  // to calculate: console.logBytes(type(VRFV2_5_ConsumerExample).creationCode);
  ...
  VRFV2_5_ConsumerExample s_testConsumer;
  ...
  function setUp() public override {
    ...
    s_testConsumer = VRFV2_5_ConsumerExample(consumerCreate2Address);
  ...
  function skipped_testRequestAndFulfillRandomWordsNative() public {
    ...
    emit RandomWordsRequested(
      ...
      1, // numWords
      VRFV2_5_Client._argsToBytes(VRFV2_5_Client.ExtraArgsV1({nativePayment: true})), // nativePayment
    ...
    // Fulfill the request.
    // Proof generated via the generate-proof-v2-5 script command. Example usage:
    /*
       go run . generate-proof-v2-5 \
  ...
  function skipped_testRequestAndFulfillRandomWordsLINK() public {
    ...
    emit RandomWordsRequested(
      ...
      1_000_000, // callbackGasLimit
      1, // numWords
      VRFV2_5_Client._argsToBytes(VRFV2_5_Client.ExtraArgsV1({nativePayment: false})), // nativePayment, // nativePayment
      address(s_testConsumer) // requester
    );
    ...
    */
    ...
    VRFCoordinatorV2_5.RequestCommitment memory rc = VRFCoordinatorV2_5.RequestCommitment({
      ...
      numWords: 1,
      sender: address(s_testConsumer),
      extraArgs: VRFV2_5_Client._argsToBytes(VRFV2_5_Client.ExtraArgsV1({nativePayment: false}))
    });
