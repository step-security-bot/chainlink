...
contract VRFV2_5_SubscriptionAPITest is BaseTest {
  ...
  function testOracleWithdrawNativeSufficientBalance() public {
    ...
    // assert state of subscription api
    assertEq(s_subscriptionAPI.getWithdrawableNativeTestingOnlyXXX(oracle), 0, "oracle withdrawable eth incorrect");
    // assert that total balance is changed by the withdrawn amount
    assertEq(s_subscriptionAPI.s_totalNativeBalance(), 9 ether, "total eth balance incorrect");
  }
